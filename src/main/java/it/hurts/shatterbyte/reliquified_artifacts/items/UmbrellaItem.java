...
public class UmbrellaItem extends RARelicItem {
                ...
                .abilities(AbilitiesTemplate.builder()
                        .ability(AbilityTemplate.builder("glider")
                                ...
                                .stat(AbilityStatTemplate.builder("bounces")
                                        .initialValue(1D, 3D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 15.0015D)
                                        .formatValue(value -> (int) MathUtils.round(value, 0))
                                        .build())
                                .stat(AbilityStatTemplate.builder("strength")
                                        .initialValue(0.5D, 0.75D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 0.99938D)
                                ...
                                .stat(AbilityStatTemplate.builder("fall_speed")
                                        ...
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 0.15033D)
                        ...
                        .ability(AbilityTemplate.builder("shield")
                                ...
                                .stat(AbilityStatTemplate.builder("hits")
                                        .initialValue(2D, 5D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 25.0025D)
                                        .formatValue(value -> (int) MathUtils.round(value, 0))
                                        .build())
                                .stat(AbilityStatTemplate.builder("repel_distance")
                                        .initialValue(0.5D, 1D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 2.001D)
                                        .formatValue(value -> MathUtils.round(value, 2))
                                        .build())
                                .stat(AbilityStatTemplate.builder("cooldown")
                                        .initialValue(10D, 15D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 0.9825D)
    ...
    private boolean canUseBounce(Player player, ItemStack stack) {
        ...
        return ability.canPlayerUse(player) && ability.getRankModifierData("bounce").isEnabled();
    }

    private void applyVanishingOnBounce(Player player, ItemStack stack) {
        var ability = this.getRelicData(player, stack).getAbilitiesData().getAbilityData("glider");

        if (!ability.canPlayerUse(player) || !ability.getRankModifierData("vanishing").isEnabled())
            return;
    ...
    private int getMaxBounces(Player player, ItemStack stack) {
        ...
        if (!ability.canPlayerUse(player) || !ability.getRankModifierData("bounce").isEnabled())
            return 0;
    ...
    private int getBounceCount(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_BOUNCE_COUNT.get(), 0);
    }

    private void setBounceCount(ItemStack stack, int count) {
        stack.set(RADataComponent.UMBRELLA_BOUNCE_COUNT.get(), Math.max(0, count));
    }

    private boolean isBounceLandingRequired(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_BOUNCE_NEEDS_LANDING.get(), false);
    }

    private void setBounceLandingRequired(ItemStack stack, boolean value) {
        stack.set(RADataComponent.UMBRELLA_BOUNCE_NEEDS_LANDING.get(), value);
    }

    private int getMaxBounces(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_MAX_BOUNCES.get(), 0);
    }

    private void setMaxBounces(ItemStack stack, int maxBounces) {
        stack.set(RADataComponent.UMBRELLA_MAX_BOUNCES.get(), Math.max(0, maxBounces));
    }

    private int getShieldHitCount(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SHIELD_HITS.get(), 0);
    }

    private void setShieldHitCount(ItemStack stack, int count) {
        stack.set(RADataComponent.UMBRELLA_SHIELD_HITS.get(), Math.max(0, count));
    }

    private int getShieldMaxHits(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SHIELD_MAX_HITS.get(), 0);
    }

    private void setShieldMaxHits(ItemStack stack, int maxHits) {
        stack.set(RADataComponent.UMBRELLA_SHIELD_MAX_HITS.get(), Math.max(0, maxHits));
    }

    private boolean isShowingShieldBar(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SHOW_SHIELD_BAR.get(), false);
    }

    private void setShowShieldBar(ItemStack stack, boolean value) {
        stack.set(RADataComponent.UMBRELLA_SHOW_SHIELD_BAR.get(), value);
    }

    private int getFallTicks(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_FALL_TICKS.get(), 0);
    }

    private void setFallTicks(ItemStack stack, int ticks) {
        stack.set(RADataComponent.UMBRELLA_FALL_TICKS.get(), Math.max(0, ticks));
    }

    private long getSlowFallProcessedTick(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SLOW_FALL_PROCESSED_TICK.get(), Long.MIN_VALUE);
    }

    private void setSlowFallProcessedTick(ItemStack stack, long tick) {
        stack.set(RADataComponent.UMBRELLA_SLOW_FALL_PROCESSED_TICK.get(), tick);
    }

    private long getSlowFallPosTick(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SLOW_FALL_POS_TICK.get(), Long.MIN_VALUE);
    }

    private void setSlowFallPosTick(ItemStack stack, long tick) {
        stack.set(RADataComponent.UMBRELLA_SLOW_FALL_POS_TICK.get(), tick);
    }

    private double getSlowFallPosX(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SLOW_FALL_POS_X.get(), 0D);
    }

    private void setSlowFallPosX(ItemStack stack, double value) {
        stack.set(RADataComponent.UMBRELLA_SLOW_FALL_POS_X.get(), value);
    }

    private double getSlowFallPosY(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SLOW_FALL_POS_Y.get(), 0D);
    }

    private void setSlowFallPosY(ItemStack stack, double value) {
        stack.set(RADataComponent.UMBRELLA_SLOW_FALL_POS_Y.get(), value);
    }

    private double getSlowFallPosZ(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.UMBRELLA_SLOW_FALL_POS_Z.get(), 0D);
    }

    private void setSlowFallPosZ(ItemStack stack, double value) {
        stack.set(RADataComponent.UMBRELLA_SLOW_FALL_POS_Z.get(), value);
    }
    ...
    public static class CommonEvents {
        ...
        public static void onShieldBlock(LivingShieldBlockEvent event) {
            ...
            if (ability.getRankModifierData("repel").isEnabled() && event.getDamageSource().getDirectEntity() instanceof LivingEntity target && target != player) {

...
public class PickaxeHeaterItem extends RAWearableRelicItem {
    @Override
                ...
                .abilities(AbilitiesTemplate.builder()
                        .ability(AbilityTemplate.builder("heating")
                                ...
                                .stat(AbilityStatTemplate.builder("fortune_chance")
                                        ...
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 0.25015D)
                                        .formatValue(value -> (int) MathUtils.round(value * 100D, 0))
                                        .build())
                                .stat(AbilityStatTemplate.builder("fortune_max_casts")
                                        .initialValue(1D, 3D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 6.003D)
    ...
    public int getFortuneLevel(SlotContext slotContext, LootContext lootContext, ItemStack stack) {
        ...
        if (!ability.canPlayerUse(player) || !ability.getRankModifierData("molten_luck").isEnabled()) {
            clearPendingSmeltRoll(stack);
            return baseFortune;
        }
        ...
        if (ability.getRankModifierData("overheat").isEnabled())
    ...
    private int getSmeltPityStacks(ItemStack stack) {
        return Math.max(0, stack.getOrDefault(RADataComponent.PICKAXE_HEATER_SMELT_PITY_STACKS.get(), 0));
    }

    private void setSmeltPityStacks(ItemStack stack, int value) {
        stack.set(RADataComponent.PICKAXE_HEATER_SMELT_PITY_STACKS.get(), Math.max(0, value));
    }

    private int getPendingSmeltResult(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.PICKAXE_HEATER_PENDING_SMELT_RESULT.get(), -1);
    }

    private void setPendingSmeltResult(ItemStack stack, int value) {
        stack.set(RADataComponent.PICKAXE_HEATER_PENDING_SMELT_RESULT.get(), value);
    }

    private int getPendingFortuneBonus(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.PICKAXE_HEATER_PENDING_FORTUNE_BONUS.get(), -1);
    }

    private void setPendingFortuneBonus(ItemStack stack, int value) {
        stack.set(RADataComponent.PICKAXE_HEATER_PENDING_FORTUNE_BONUS.get(), value);
    }
    ...
    public static class CommonEvents {
        ...
        public static void onLivingIncomingDamageDealing(LivingIncomingDamageEvent event) {
            ...
            for (var stack : EntityUtils.findEquippedCurios(player, ModItems.PICKAXE_HEATER.value())) {
                ...
                if (!ability.canPlayerUse(player) || !ability.getRankModifierData("blazing_strike").isEnabled())
                    continue;

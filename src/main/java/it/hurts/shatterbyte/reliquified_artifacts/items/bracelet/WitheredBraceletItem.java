...
public class WitheredBraceletItem extends RAWearableRelicItem {
    @Override
    ...
    private static boolean applyWitherFromBracelet(LivingEntity target, Player owner, int ticks, int amplifier) {
        ...
        if (data.hasUUID("reliquified_artifacts_withered_bracelet_owner") && owner.getUUID().equals(data.getUUID("reliquified_artifacts_withered_bracelet_owner")))
            expireTick = Math.max(expireTick, data.getLong("reliquified_artifacts_withered_bracelet_expire"));

        data.putUUID("reliquified_artifacts_withered_bracelet_owner", owner.getUUID());
        data.putLong("reliquified_artifacts_withered_bracelet_expire", expireTick);
    ...
    private static Player getActiveOwner(LivingEntity target) {
        ...
        if (!data.hasUUID("reliquified_artifacts_withered_bracelet_owner"))
            return null;

        var expireTick = data.getLong("reliquified_artifacts_withered_bracelet_expire");
        ...
        return server.getPlayerList().getPlayer(data.getUUID("reliquified_artifacts_withered_bracelet_owner"));
    }

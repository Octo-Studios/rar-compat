...
public class PowerGloveItem extends RAWearableRelicItem {
    @Override
                ...
                .abilities(AbilitiesTemplate.builder()
                        .ability(AbilityTemplate.builder("power")
                                ...
                                .stat(AbilityStatTemplate.builder("hits_required")
                                        .initialValue(10D, 7D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 3.0065D)
                                        .formatValue(value -> (int) MathUtils.round(value, 0))
                                        .build())
                                .stat(AbilityStatTemplate.builder("power_damage_bonus")
                                        .initialValue(1D, 1.5D)
                                        .targetValue(RelicsScalingModels.MULTIPLICATIVE_BASE.get(), 5.00175D)
    ...
    private int getHitCounter(ItemStack stack) {
        return Math.max(0, stack.getOrDefault(RADataComponent.POWER_GLOVE_HIT_COUNTER.get(), 0));
    }

    private void setHitCounter(ItemStack stack, int value) {
        stack.set(RADataComponent.POWER_GLOVE_HIT_COUNTER.get(), Math.max(0, value));
    }

    private boolean getForceNext(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.POWER_GLOVE_FORCE_NEXT.get(), false);
    }

    private void setForceNext(ItemStack stack, boolean value) {
        stack.set(RADataComponent.POWER_GLOVE_FORCE_NEXT.get(), value);
    }

    private boolean getPowerStrikeActive(ItemStack stack) {
        return stack.getOrDefault(RADataComponent.POWER_GLOVE_POWER_STRIKE_ACTIVE.get(), false);
    }

    private void setPowerStrikeActive(ItemStack stack, boolean value) {
        stack.set(RADataComponent.POWER_GLOVE_POWER_STRIKE_ACTIVE.get(), value);
    }

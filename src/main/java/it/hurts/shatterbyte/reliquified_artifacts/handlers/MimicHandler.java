...
public class MimicHandler {
    ...
    public static void onStartedServer(TagsUpdatedEvent event) {
        var mimicLoot = BuiltInRegistries.ITEM.getTag(TagKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath(ReliquifiedArtifacts.MODID, "mimic_loot")))
                .stream().flatMap(holderSet -> holderSet.stream().map(Holder::value)).toList();
        var mimificable = BuiltInRegistries.ITEM.getTag(TagKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath(ReliquifiedArtifacts.MODID, "mimificable")))
                .stream().flatMap(holderSet -> holderSet.stream().map(Holder::value)).toList();
